# D8 Dice roller script - attach to a Control node
...
@onready var dice = $PanelContainer/VBoxContainer/SubViewportContainer/SubViewport/D8
...
func _ready() -> void:
	...
	print("D8 Dice node found: ", dice != null)
	if dice:
		print("D8 children count: ", dice.get_child_count())
		print("D8 children: ", dice.get_children())
	...
	for i in range(1, 9):
		...
		if side_node:
			...
			push_error("Side" + str(i) + " node not found! Make sure all Side1-Side8 nodes exist as children of D8")
	
	if sides.size() != 8:
		push_error("Not all side markers found! Found " + str(sides.size()) + " out of 8")
	else:
		print("All 8 sides found successfully!")
