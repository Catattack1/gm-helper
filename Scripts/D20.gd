# D20 Dice roller script - attach to a Control node
...
@onready var dice = $PanelContainer/VBoxContainer/SubViewportContainer/SubViewport/D20
...
func _ready() -> void:
	...
	# Debug: Print dice node info
	print("D20 Dice node found: ", dice != null)
	if dice:
		print("D20 children count: ", dice.get_child_count())
		print("D20 children: ", dice.get_children())
	...
	for i in range(1, 21):
		...
		if side_node:
			sides[i] = side_node
			print("Found Side", i, " at position: ", side_node.position)
		else:
			push_error("Side" + str(i) + " node not found! Make sure all Side1-Side20 nodes exist as children of D20")
	
	if sides.size() != 20:
		push_error("Not all side markers found! Found " + str(sides.size()) + " out of 20")
	else:
		print("All 20 sides found successfully!")
...
func _detect_and_center_result():
	...
	for side_num in sides.keys():
		...
		if side_node and is_instance_valid(side_node):
			...
			print("Side ", side_num, " - Direction: ", side_direction, " Dot: ", dot)
	...
	print(">>> DETECTED SIDE: ", closest_side, " with dot product: ", closest_dot, " <<<")
...
func _calculate_face_alignment(face: int) -> Vector3:
	...
	var rotation_axis = current_side_dir.cross(target_dir)
	
	if rotation_axis.length() > 0.001:  # Avoid zero-length axis
		rotation_axis = rotation_axis.normalized()
		var rotation_angle = current_side_dir.angle_to(target_dir)
		
		# Create a basis from the rotation and apply it to current rotation
		var current_basis = dice.transform.basis
		var rotation_basis = Basis(rotation_axis, rotation_angle)
		var final_basis = rotation_basis * current_basis
		
		return final_basis.get_euler()
	else:
