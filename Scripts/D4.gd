# D4 Dice roller script - attach to a Control node
# This detects which side is facing UP after rolling (not toward camera)
extends Control
...
@onready var dice = $PanelContainer/VBoxContainer/SubViewportContainer/SubViewport/D4
...
func _ready() -> void:
	...
	for i in range(1, 5):  # D4 has 4 sides
		var side_node = dice.get_node_or_null("Side" + str(i))
		if side_node:
			sides[i] = side_node
		else:
			push_error("Side" + str(i) + " node not found! Make sure all Side1-Side4 nodes exist as children of D4")
	
	if sides.size() != 4:
		push_error("Not all side markers found! Found " + str(sides.size()) + " out of 4")
...
func _detect_and_center_result():
	...
	# For D4, we check which side is facing UP (positive Y direction)
	var up_direction = Vector3.UP
	var closest_side = 1
	var closest_dot = -999.0
	
	# Find which side is facing up most directly
	for side_num in sides.keys():
		...
		if side_node and is_instance_valid(side_node):
			...
			# Check how aligned this side is with the up direction
			var dot = side_direction.dot(up_direction)
	...
	# Now calculate rotation needed to orient this face upward
	var side_node = sides[closest_side]
	if side_node and is_instance_valid(side_node):
		var side_direction = (side_node.global_position - dice.global_position).normalized()
		
		# Calculate rotation to align this face with up
...
# Calculate the rotation needed to face a specific side upward
func _calculate_face_alignment(face: int) -> Vector3:
	...
	var target_dir = Vector3.UP  # Point upward instead of toward camera
