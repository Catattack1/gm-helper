# D12 Dice roller script - attach to a Control node
...
@onready var dice = $PanelContainer/VBoxContainer/SubViewportContainer/SubViewport/D12
...
func _ready() -> void:
	...
	print("D12 Dice node found: ", dice != null)
	if dice:
		print("D12 children count: ", dice.get_child_count())
		print("D12 children: ", dice.get_children())
	...
	for i in range(1, 13):
		...
		if side_node:
			...
			push_error("Side" + str(i) + " node not found! Make sure all Side1-Side12 nodes exist as children of D12")
	
	if sides.size() != 12:
		push_error("Not all side markers found! Found " + str(sides.size()) + " out of 12")
	else:
		print("All 12 sides found successfully!")
