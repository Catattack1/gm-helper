# D10 Dice roller script - attach to a Control node
...
@onready var dice = $PanelContainer/VBoxContainer/SubViewportContainer/SubViewport/D10
...
func _ready() -> void:
	...
	print("D10 Dice node found: ", dice != null)
	if dice:
		print("D10 children count: ", dice.get_child_count())
		print("D10 children: ", dice.get_children())
	...
	for i in range(1, 11):
		...
		if side_node:
			...
			push_error("Side" + str(i) + " node not found! Make sure all Side1-Side10 nodes exist as children of D10")
	
	if sides.size() != 10:
		push_error("Not all side markers found! Found " + str(sides.size()) + " out of 10")
	else:
		print("All 10 sides found successfully!")
